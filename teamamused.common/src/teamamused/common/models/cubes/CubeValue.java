...
public class CubeValue {
	...
	return this.Color == otherCubeValue.Color && this.FaceValue == otherCubeValue.FaceValue;
    }
	...
	if (this.FaceValue == 0) {
		...
		g.drawImage(this.SpecialCard.getCubeSymbol(), randUmsBild, randUmsBild, canvasWidth
			- (randUmsBild * 2), canvasWidth - (randUmsBild * 2));
	    }
		...
		g.fillOval(border + rightAndBottomDrawStart, border + rightAndBottomDrawStart, pointSize,
			pointSize);
	    }
	    if (this.FaceValue > 3) {
		// Oben Rechts und unten Links für die Zahlen 4 und 5
		g.fillOval(border + rightAndBottomDrawStart, border + leftAndUperDrawStart, pointSize,
			pointSize);
		g.fillOval(border + leftAndUperDrawStart, border + rightAndBottomDrawStart, pointSize,
			pointSize);
	    }
